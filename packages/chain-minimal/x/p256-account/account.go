...
import (
	...
	v1 "github.com/cosmosregistry/chain-minimal/x/p256-account/v1"
...
func (a Account) Authenticate(ctx context.Context, msg *aa_interface_v1.MsgAuthenticate) (*aa_interface_v1.MsgAuthenticateResponse, error) {
	...
	clientDataJSON, authenticatorData, passkeySig, err := parseWebAuthnSignature(signature, signBytes)
	if err != nil {
		return nil, fmt.Errorf("failed parsing WebAuthn signature: %w", err)
	}

	// Compute actual signature from WebAuthn components
	sigHash, err := computeWebAuthnSignature(clientDataJSON, authenticatorData)
	if err != nil {
		return nil, fmt.Errorf("failed computing WebAuthn signature: %w", err)
	}

	if !pubKey.VerifySignature(sigHash, passkeySig) {
