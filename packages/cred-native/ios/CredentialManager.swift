...
@available(iOS 16.0, *)
class CredentialManager: NSObject, ASAuthorizationControllerPresentationContextProviding, ASAuthorizationControllerDelegate {
  ...
  func authenticate(
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      guard let challengeData = Data.fromBase64url(base64Data: getOptions.challenge) else {
        continuation.resume(throwing: CustomErrors.base64UrlDecodingError(reason: "challenge could not be decoded: Invalid base64url"))
        kontinuation = nil
        return
      }
      
      let provider = ASAuthorizationPlatformPublicKeyCredentialProvider(relyingPartyIdentifier: getOptions.rpId)
      let request = provider.createCredentialAssertionRequest(challenge: challengeData)
      
      request.userVerificationPreference = ASAuthorizationPublicKeyCredentialUserVerificationPreference(rawValue: getOptions.userVerification!)
      
      
      let cpProvider = ASAuthorizationSecurityKeyPublicKeyCredentialProvider(relyingPartyIdentifier: getOptions.rpId)
      ...
      let authController = ASAuthorizationController(authorizationRequests: [request, cpRequest])
  ...
  func register(
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      guard let userIDData = Data.fromBase64url(base64Data: createOptions.user.id) else {
        continuation.resume(throwing: CustomErrors.base64UrlDecodingError(reason: "user.id could not be decoded: Invalid base64url"))
        kontinuation = nil
        return
      }
      
      @MainActor func getRequest() -> ASAuthorizationPublicKeyCredentialRegistrationRequest {
        if createOptions.authenticatorSelection.authenticatorAttachment == Konstants.AUTHENTICATOR_ATTACHMENT {
          // platfrom request
          let provider = ASAuthorizationPlatformPublicKeyCredentialProvider(relyingPartyIdentifier: createOptions.rp.id)
          ...
          if let excludedCredentials = createOptions.excludeCredentials, !excludedCredentials.isEmpty {
          ...
          let provider = ASAuthorizationSecurityKeyPublicKeyCredentialProvider(relyingPartyIdentifier: createOptions.rp.id)
          let request = provider.createCredentialRegistrationRequest(
            challenge: challengeData,
          ...
          request.residentKeyPreference = ASAuthorizationPublicKeyCredentialResidentKeyPreference(rawValue: createOptions.authenticatorSelection.residentKey)
          request.credentialParameters = createOptions.pubKeyCredParams.map {
            credParam in ASAuthorizationPublicKeyCredentialParameters(algorithm: ASCOSEAlgorithmIdentifier(credParam.alg))
          }
      ...
      request.userVerificationPreference = ASAuthorizationPublicKeyCredentialUserVerificationPreference(rawValue: createOptions.authenticatorSelection.userVerification)
      
      let authController = ASAuthorizationController(authorizationRequests: [request as! ASAuthorizationRequest])
      authController.delegate = self
      authController.presentationContextProvider = self
  ...
  func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
    ...
    case let credentialAssertion as ASAuthorizationPlatformPublicKeyCredentialAssertion:
      ...
      kontinuation = nil
    default:
      kontinuation?.resume(throwing: CustomErrors.unexpectedAuthorizationResponse(credential: authorization.credential))
      kontinuation = nil
    }
  ...
  private func parseCredentials<T: ASAuthorizationPublicKeyCredentialDescriptor>(
    ...
    return credentials.compactMap { credential in
      ...
      if descriptorType == ASAuthorizationPlatformPublicKeyCredentialDescriptor.self {
        return ASAuthorizationPlatformPublicKeyCredentialDescriptor(credentialID: credentialID) as? T
      } else if descriptorType == ASAuthorizationSecurityKeyPublicKeyCredentialDescriptor.self {
        let transports = credential.transports?.compactMap { channel in
          ASAuthorizationSecurityKeyPublicKeyCredentialDescriptor.Transport(rawValue: channel)
        } ?? [.bluetooth, .nfc, .usb]
        return ASAuthorizationSecurityKeyPublicKeyCredentialDescriptor(credentialID: credentialID, transports: transports) as? T
      } else {
        return nil
      }
    }
